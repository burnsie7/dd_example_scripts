...
def get_timeboard(dashid):
    tb = api.Timeboard.get(dashid)
    print(tb)
    filename = 'timeboard_' + str(dashid) + '.json'
    file = open(filename, "w")
    file.write(json.dumps(tb, indent=4, sort_keys=True))
...
if __name__ == "__main__":
    ...
    if not dashid:
        errors.append("""
                      You must supply a timeboard id by passing a
                      -d/--dashid argument""")
    if errors:
        for error in errors:
            print(error)
